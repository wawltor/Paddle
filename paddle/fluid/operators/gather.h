...
void CPUMaxGather(const platform::DeviceContext& ctx, 
  ...
  T* data_output_x = output_x->data<T>();
  T* data_output_u = output_u->data<T>();
  // slice size
  int slice_size = 1;
  for (int i = 1; i < src_dims.size(); ++i) slice_size *= src_dims[i];
  ...
  for (int64_t i = 0; i < index_size; ++i) {
    ...
    for (int j = 0; j < slice_size; ++j) {
      ...
      if (is_big) {
        data_output_u[i * slice_size + j] = static_cast<T>(0);
      } else {
        T tmp_value = data_output_x[index_ * slice_size + j];
        data_output_x[index_ * slice_size + j] = static_cast<T>(0); 
        data_output_u[i * slice_size + j] = tmp_value;
      }
    }
  }
}
