...
void ScatterAssignAdd(const framework::ExecutionContext& ctx, const Tensor& src,
  ...
  //Use the JIT to speed up add op
  //Use the 3 * slice_size as code key 
  auto compute = jit::KernelFuncs<jit::VAddTuple<T>, platform::CPUPlace>::Cache().At(slice_size * 3);
  for (int i = 0; i < index_size; ++i) {
    const IndexT& index_ = p_index[i];
    ...
    if (overwrite) {
       ...
       //elementwise_inner_add<T, IndexT>(ctx, src, output, i, index_, slice_size,
                                       //slice_bytes);
       compute(p_src + i * slice_size, p_output +  index_ * slice_size, 
              p_output +  index_ * slice_size, slice_size);
    }
  }
}
